...
train_lfcc_path = os.path.join(out_dir, "train_lfcc.npy")
train_labels_path = os.path.join(out_dir, "train_labels.npy")
dev_lfcc_path = os.path.join(out_dir, "dev_lfcc.npy")
dev_labels_path = os.path.join(out_dir, "dev_labels.npy")

for p in [train_lfcc_path, train_labels_path, dev_lfcc_path, dev_labels_path]:
    if not os.path.exists(p):
        raise FileNotFoundError(f"Missing: {p}")


class MemmapLFCCDataset(Dataset):
    def __init__(self, lfcc_path, labels_path, augment=False):
        self.X = np.load(lfcc_path, mmap_mode="r")
    ...
    def __getitem__(self, idx):
        ...
        if self.augment:
            ...
            if np.random.rand() < 0.5:
                f_w = np.random.randint(1, max(2, int(0.1 * x.shape[0])))
...
sample_weights = torch.from_numpy(class_weights[labels_arr].astype(np.float32))
sampler = WeightedRandomSampler(sample_weights, len(sample_weights), replacement=True)

train_ds = MemmapLFCCDataset(train_lfcc_path, train_labels_path, augment=True)
dev_ds = MemmapLFCCDataset(dev_lfcc_path, dev_labels_path, augment=False)
...
logger.info(f"Class balance — bonafide: {class_counts[0]}, spoof: {class_counts[1]}")
...
logger.info(f"LightCNN parameters: {n_params:,}")
